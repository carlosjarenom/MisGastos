...
def init_db():
    ...
    # Obtener IDs válidos dinámicamente
    valid_ids = [cat[0] for cat in CATEGORIES + TRANSPORT_SUBCATEGORIES]
    if not valid_ids:
        valid_ids = [6] # Fallback a Otros si está vacío (no debería pasar)

    valid_ids_str = ",".join(map(str, valid_ids))
    fallback_id = valid_ids[-1] # Usar la última como fallback (típicamente 'Otros')

    # Eliminar categorías que ya no existen en config.py
